			...
			u=function(t,n){
				return function(i){
					...
					s.settings.$node.attr({
						id:s.settings.id,role:"checkbox","aria-checked":false,
						focusable:true,
						tabIndex:-1
					}).text(s.settings.label).addClass(
							["seatCharts-seat","seatCharts-cell","available"].concat(
							     s.settings.classes,
							...
							s.status=function(){
								...
								(function(i,o,u){e.each(["click","focus","blur"],function(e,a){
									s[a]=function(){
									if(a=="focus"){
										...
										u.node().focus()}return s.style(typeof i[o][a]==="function"?i[o][a].apply(u):n[a].apply(u))}})})(n.seats,s.settings.character,s);s.node().on("click",s.click).on("mouseenter",s.focus).on("mouseleave",s.blur).on("keydown",function(e,n){
											return function(i){
												var s;
												switch(i.which){
													case 32:i.preventDefault();
													e.click();
													break;case 40:case 38:i.preventDefault();
						    s=function o(e,t,r){
							var u;
